...
const PRODUCTS = [
  { id: 1, name: 'Mövenpick Schoki', emoji: '🍫', price: 0 },
  { id: 2, name: 'Mövenpick Vanilla', emoji: '🍦', price: 0 },
  { id: 3, name: 'PassionFruit & Coconut', emoji: '🥥', price: 1.00 }
];
...
export default function App() {
  ...
  const handleOrder = async (product) => {
    ...
    try {
      ...
      if (!cust) {
        const { data: newCust } = await supabase.from('customers').insert([{ phone, email, address, plz_city: '', name: phone }]).select().single();
        cust = newCust;
      }
      if (cust) await supabase.from('orders').insert([{ customer_id: cust.id, product_id: product.id }]);
          ...
          <div style={{display:'flex',gap:'0.4rem'}}>
            {free.map(p => (
              <button key={p.id} onClick={()=>handleOrder(p)} disabled={loading}
                ...
                <span style={{fontSize:'0.65rem',fontWeight:'600'}}>{p.name.split(' ')[1]}</span>
              </button>
            ))}
          </div>
        </div>
